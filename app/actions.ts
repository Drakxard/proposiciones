...
const VARIANT_LABELS = GROQ_VARIANT_LABELS

export async function generateProposition(
  condition: string,
  ...
  try {
    console.log("[v0] Generating proposition for:", condition, "type:", variant)
    ...
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Condición base: ${condition}\nTipo solicitado: ${VARIANT_LABELS[variant]}\nGenera únicamente la proposición correspondiente.`,
          },
        ],
        temperature: 0.7,
        max_tokens: 512,
    ...
    console.error("[v0] Error generating proposition:", error)
    return { error: `Error al generar la proposición: ${error instanceof Error ? error.message : String(error)}` }
  }
}
...
export async function getAvailableModels(): Promise<string[]> {
  ...
  if (!apiKey) {
    console.warn("[v0] GROQ_API_KEY no está configurada, devolviendo modelo por defecto")
